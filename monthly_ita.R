...
it.extent <- ext(xmin(it.monthly), xmax(it.monthly), ymin(it.monthly), ymax(it.monthly)-res(it.monthly)[2])
clino.it.mmean <- crop(it.monthly, it.extent)
writeRaster(clino.it.mmean, paste0(path.temp, "temp_month.tif"), overwrite = T)
### From cache
clino.it.mmean <- rast(paste0(path.temp, "temp_month.tif"))
# Upscaling
clino.it.mmean.agg <- aggregate(clino.it.mmean, 12, "mean", na.rm = T)
...
# indici per raggruppare sui mesi con stackApply
...
# eobs.resampled.monthly.cumulated <- tapp(eobs.subset.resampled, index, "sum", filename = paste0(path.temp, "temp_cum_month_resamp.grd"), overwrite = T)
# Cache:
# eobs.monthly.cumulated <- brick(paste0(path.temp, "temp_cum_month.grd"))
...
plot(clino.it.mmean.agg - shift(eobs.it.mmean, dx, dy), col = rev(diverging_hcl(51, palette = "Vik")), range = c(-150, 150))
dev.off()
